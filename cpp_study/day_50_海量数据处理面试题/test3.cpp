 ...
 *本题还是用两个位来标记，分为出现0次 00 表示，出现1次的 01 表示 出现2次的 10
 *表示 出现3次及3次以上的 11 表示 位图只能处理整数 布隆过滤器
 *分析：（query一般就是sql查询语句或者网络请求的url，一般是一个字符串）
 *1.给两个文件，分别有100亿个query，我们只有1g内存，如何找到两个文件的交集
 *分别给出精确算法和近似算法 2.如何扩展bloomfilter，使其支持删除操作
 *100亿个query占用多少查询 假设平均一个query 30-60byte，100亿个query占用300 -
 *600g内存
 ...
 *A0   A1   A2   A999
 *B0   B1   B2   B999
 * 切分成多少份:一般切出来一个小文件的大小可以放进内存就可以。那么这里一个文件300-600g，那么切分成1000份
 *一个文件300-600m
